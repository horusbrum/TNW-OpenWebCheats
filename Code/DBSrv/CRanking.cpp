...
void RankingSystem::loadRanking()
{
	...
	unsigned int startClock = GetTickCount64();
	int entryCount = readAccountsInDir("./account/*");
	unsigned int resultClock = GetTickCount64() - startClock;
...
*/
int RankingSystem::tryInsertInRanking(int rankId, char* name, long long value, int classvalue, int Level, int cls, GrindRankingConnId* connId, STRUCT_RANKING* olderLastRank, GrindRankingConnId* olderLastConnId, int g , int clan)
{
	...
	if (rankId == OUT_OF_RANK)
	{
		if (value > grindRanking.getElement(LAST)->Value && classvalue >= grindRanking.getElement(LAST)->ClassMaster || classvalue > grindRanking.getElement(LAST)->ClassMaster)
		{
			...
			*/
			...
			STRUCT_RANKING* newRank = new STRUCT_RANKING(name, value, classvalue, Level, cls, g, clan);
