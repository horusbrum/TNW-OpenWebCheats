...
void Msg_HNC_Br(){
	MessageBoxA(NULL, "XVI\n\nConteúdo suspeito detectado!5", carrega.Nome_das_Janelas, MB_SERVICE_NOTIFICATION | MB_ICONWARNING);
}
void Msg_HNC_En(){
		...
		for (j = 0; j <= BadWordsCount - 1; j++)			//scan badwords 
		{
			if (strcmp(Anti_BadWord[j].BadWords, Word_Part[i].Parts) == 0 || strcmp(Anti_BadWord[j].BadWords, ClassName) == 0)
			{
				if (carrega.Log_Txt_Hack == 1){
					...
					out << "\n XVI:  ", out << windowName;    // printf ("Found Hack = %s " , windowName);
					out.close();
				}
