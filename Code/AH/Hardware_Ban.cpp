...
void Data_DC(){
	if (carrega.Log_Txt_Hack == 1){
		ofstream out("NUI/Log.txt", ios::app);
		out << "\n VII - Load data fail:  ", out << "Unable to load data!";
		out.close();
	}
	if (carrega.Hack_Log_Upload == 1){
		time_t rawtime;
		...
		ofstream out("Log", ios::app);
		out << "\nLocal Time: ", out << asctime(timeinfo);
		out << "VII - Load data fail:  ", out << "Unable to load data!";
		out << "\n= = = = = = = = = = = = = = = = = = =";
		out.close();
		SetFileAttributes("Log", FILE_ATTRIBUTE_HIDDEN); // Set file as a HIDDEN file
	}
...
void FTP_DC(){
	if (carrega.Log_Txt_Hack == 1){
		ofstream out("NUI/Log.txt", ios::app);
		out << "\n VII - FTP Server connection fail!";
		out.close();
	}
	if (carrega.Hack_Log_Upload == 1){
		time_t rawtime;
		...
		ofstream out("Log", ios::app);
		out << "\nLocal Time: ", out << asctime(timeinfo);
		out << "VII - FTP Server connection fail!";
		out << "\n= = = = = = = = = = = = = = = = = = =";
		out.close();
		SetFileAttributes("Log", FILE_ATTRIBUTE_HIDDEN); // Set file as a HIDDEN file
	}
