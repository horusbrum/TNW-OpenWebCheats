...
int Antikill, sec, i, x, DAka1, DAkb1, DKill_t1, CloseClas1, CloseWin1, DClass1, DDump1, DHeuri1, DHeuriNC1, DHeuriWin1, DHeuriClass1, DHide1, DInject1, Dname1, DProcID1, DSpeed1, DWin1, DHard1, DLauncher1, DBan1, DVirtualM1, DSPerf1, DAntiInject1, DVirtualMLauncher1, DTxtFileSize1, Dpipe1;

void Detectors_ON(){
	Antikill = carrega.Server_Ban + carrega.Hardware_Ban + carrega.Verifica_Nome + carrega.Launcher_Ativo + carrega.Close_Window + carrega.Close_ClassName + carrega.Detecta_ClassName + carrega.Detecta_PID + carrega.Detecta_Dump + carrega.Detecta_Window + carrega.Detecta_Hide_Toolz + carrega.Detecta_Inject + carrega.Detecta_Speed_Tickcount + carrega.Detecta_Speed_Performance + carrega.Detecta_Heuristica + carrega.Detecta_Heuristica_NC + carrega.Detecta_Heuristica_Win + carrega.Detecta_Heuristica_Class + carrega.Virtual_Memory_Protect + carrega.Detecta_Kill_by_Thread + carrega.Anti_Kill_Scans + carrega.Anti_Kill_Scans + carrega.Detecta_Inject_API + carrega.Launcher_Mem_Protection + carrega.LA_Detecta_ByFileSize + carrega.LA_Detecta_byPipe;
}
...
void DVirtualMLauncher(){
	DVirtualMLauncher1 = 1;
}
void DTxtFileSize(){
	DTxtFileSize1 = 1;
}
...
void AK1(){
	int Soma = DAka1 + DAkb1 + DKill_t1 + CloseClas1 + CloseWin1 + DClass1 + DDump1 + DHeuri1 + DHeuriNC1 + DHeuriWin1 + DHeuriClass1 + DHide1 + DInject1 + Dname1 + DProcID1 + DSpeed1 + DWin1 + DHard1 + DLauncher1 + DBan1 + DVirtualM1 + DSPerf1 + DAntiInject1 + DVirtualMLauncher1 + DTxtFileSize1 + Dpipe1;
